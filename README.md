...
TransitInk OS is an ESP32-S3 firmware for a 400x300 e-paper TTC surface arrival
dashboard, developed for the Zectrix Note 4 hardware profile. It displays four
independently configured widget slots for Toronto Transit Commission bus and
streetcar arrivals and includes an on-device English settings portal.
...
Toronto. Zectrix and its product names remain the property of their respective
owner.
...
settings, and configure up to four ordered widget slots. A slot may also be
disabled. The settings access point remains available until setup is saved or
the device is restarted.

After the device has joined its configured Wi-Fi, pressing the Volume Up button
opens the English settings portal on the device's current LAN IP instead of
creating the setup access point. The display shows the local address and a QR
code containing a random session path. The portal remains available until
settings are saved, the device is restarted, or Volume Up is pressed again. If
normal Wi-Fi is unavailable, the device falls back to the protected
`TransitInk-xxxx` access point.

TransitInk OS refreshes TTC surface arrivals every 60 seconds. The firmware
downloads the TTC BusTime GTFS-Realtime Trip Updates feed from
`https://bustime.ttc.ca/gtfsrt/trips`, filters it on-device by the configured
TTC `stop_id` and `route_id`, and shows the next one or two arrivals per active
slot.

The settings portal reads its route, direction, and stop choices from the
versioned TTC catalog embedded in the firmware under `data/catalog/ttc/`. Normal
builds never download the static catalog at runtime. Maintainers refresh the
catalog explicitly with:
...
the daily window is enabled. The schedule requires the device clock to have been
synchronized over Wi-Fi at least once since power-on.

The dashboard reserves a footer area for weather, but the TTC-only firmware does
not fetch remote weather data.

## Repository layout

```text
include/        C++ headers and board configuration
src/            firmware, TTC client, display, portal, and application entry point
src/core/       hardware-independent domain logic
src/generated/  generated glyph data and embedded TTC catalog arrays
data/catalog/   TTC catalog assets and integrity metadata
...
scripts/        setup, font/catalog generation, backup, flash, and restore tools
...
`include/hardware/boards/ZectrixNote4.h`, using an SSD1683-style 400x300 panel.
